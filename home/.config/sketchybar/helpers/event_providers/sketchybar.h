...
static inline bool mach_send_message(mach_port_t port,
                                     char *message,
                                     size_t len) {
  if (!message || !MACH_PORT_VALID(port) || len > UINT32_MAX) return false;
...
static inline void sketchybar(const char *message) {
  ...
  if (!MACH_PORT_VALID(g_mach_port)) g_mach_port = mach_get_bs_port();
  if (!mach_send_message(g_mach_port, formatted_message, length)) {
    g_mach_port = mach_get_bs_port();
    if (!mach_send_message(g_mach_port, formatted_message, length)) exit(0);
  }
}

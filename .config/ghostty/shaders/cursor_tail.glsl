...
const float MIN_DURATION = 0.1; // in seconds
const float MAX_DURATION = 0.4; // in seconds
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    float lineLength = length(delta);
     ...
     float duration = mix(MIN_DURATION, MAX_DURATION, clamp(lineLength / MAX_TRAIL_LENGTH, 0.0, 1.0));

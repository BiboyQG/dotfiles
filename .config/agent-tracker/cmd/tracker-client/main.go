...
func runUI(args []string) error {
	...
	draw := func(now time.Time) {
		...
		renderNotes := func(list []ipc.Note, state *listState, archived bool, startRow int, focused bool) int {
			...
			if helpVisible {
				helpLines := []string{
					"j/k: move | t: toggle Tracker/Notes | Tab: focus goals/notes | n/i: view scope | Alt-A: archive view",
					"Goals: a add | Enter/c: complete | Shift-D: delete (focus goals first)",
					"Notes: a add | e edit | Enter/c: complete | Shift-A: archive | Shift-D: delete | Shift-C: show/hide completed | Esc: close | ?: toggle help",
				}
				row := 3
